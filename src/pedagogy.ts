...
- Generate audio when hearing the content is part of the skill or the learner asks to hear it. Prefer a host image tool (Grok Imagine, Cursor, etc.) when seeing it helps; then import_image with the returned URL and attach that hash. Do not call generate_image when a host image URL is available. Fall back to generate_image only if no host tool is available. If the learner says an attached image is wrong: import a better host image (or generate_image with replace: true only when no host tool), then attach_image with replace: true. Do not add media merely for richness.
